...
const LaundaryInvoiceDialog = ( props ) => {
  ...
  const calculateTotalLaudaryAmount = () => {
    if ( !data || !data?.laundryDetails ) {
      return 0;
    }

    const totalLaundaryAmount = data?.laundryDetails?.reduce( ( total, LaundaryData ) => {
  ...
  useEffect( () => {
    setTotalLaundaryAmount( calculateTotalLaudaryAmount() );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ data ] );
  ...
  const validationSchema = yup.object().shape( {
    ...
    laundaryAmount: yup.number().required( 'Laundary amount is required' ).min( 0, 'Laundary amount cannot be negative' ),
  ...
  const generateLaundaryInvoice = async ( values, resetForm ) => {
    try {
      values.type = 'laundary';
      ...
      console.log( 'sumon values', values );
      const response = await postApi( 'api/invoice/add', values );
      if ( !hotelData.laundarygstpercentage ) {
        const data = {};
        data.laundarygstpercentage = formik.values.gstPercentage;
      } else {
        console.log( '' );
      }

      console.log( 're llllllllll', response );
      ...
      setTimeout( () => {
        ...
        navigate( `/separatelaundarybill/view/${ response.data._id }` );
      }, 200 );
    } catch ( error ) {
      console.log( error );
  ...
  const formik = useFormik( {
    ...
    onSubmit: async ( values, { resetForm } ) => {
      generateLaundaryInvoice( values, resetForm );
    }
  } );
  ...
  const calculateTotalAmount = () => {
    const totallaundaryAmountWithoutGst = formik.values.laundaryAmount - formik.values.discount;
    const gstAmount = totallaundaryAmountWithoutGst * ( formik.values.gstPercentage / 100 );
    setGstAmount( gstAmount );
    const totalAmount = haveGST ? totallaundaryAmountWithoutGst + gstAmount : totallaundaryAmountWithoutGst;
  ...
  useEffect( () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ formik.values.laundaryAmount, formik.values.discount, formik.values.gstPercentage, haveGST ] );
  ...
  return (
    <div>
      <Dialog open={ open } aria-labelledby="scroll-dialog-title" aria-describedby="scroll-dialog-description">
        ...
        >
          <Typography variant="h6">Laundary Invoice Information</Typography>
          <Typography>
            ...
            <Grid container rowSpacing={ 3 } columnSpacing={ { xs: 0, sm: 2 } }>
              ...
              <Grid item xs={ 12 } sm={ 6 }>
                ...
                <TextField
                  ...
                  type="number"
                  fullWidth
                  value={ formik.values.laundaryAmount }
                  onChange={ formik.handleChange }
                  error={ formik.touched.laundaryAmount && Boolean( formik.errors.laundaryAmount ) }
                  helperText={ formik.touched.laundaryAmount && formik.errors.foodAmount }
                />
              </Grid>
              {/* Discount */ }
              <Grid item xs={ 12 } sm={ 6 }>
...
LaundaryInvoiceDialog.propTypes = {
  open: PropTypes.bool.isRequired,
  handleClose: PropTypes.func.isRequired,
  data: PropTypes.object.isRequired
};

export default LaundaryInvoiceDialog;
