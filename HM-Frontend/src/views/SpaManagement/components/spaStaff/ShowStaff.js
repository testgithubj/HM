...
import ChangePasswordForStaff from './components/ChangePassword';
...
export default function SpaStaffView () {
  ...
  const fetchEmployeeData = async () => {
    try {
      const response = await getApi( `api/spaStaff/view/${ employeeId }` );
      console.log( 'datdaaaaaaaa=========>', response?.data?.data[ 0 ] );
      setEmployeeData( response?.data?.data[ 0 ] );
      setEmployeeEmail( response?.data?.data[ 0 ].email );
    } catch ( error ) {
      console.log( error );
    }
  };
                  ...
                  <Item sx={ { height: '100%' } }>
                    ...
                    <Grid container spacing={ 2 } sx={ { justifyContent: 'between', alignItems: 'center', marginTop: '1px' } }>
                      ...
                      <Grid item xs={ 6 } md={ 6 }>
                        ...
                        <Typography style={ { color: 'black', display: 'flex', flexDirection: 'column', wordWrap: 'break-word' } }>
                    ...
                    <Grid container spacing={ 2 } sx={ { justifyContent: 'between', alignItems: 'center', marginTop: '1px' } }>
                      ...
                      <Grid item xs={ 6 } md={ 6 }>
                        ...
                        { employeeData.createdDate ? (
                          <Typography variant="body1">{ moment( employeeData.createdDate ).format( 'MMMM D, YYYY' ) }</Typography>
                        ) : (
                    ...
                    <Grid container spacing={ 2 } sx={ { justifyContent: 'between', alignItems: 'center', marginTop: '1px' } }>
                      ...
                      >
                        <Grid
                          sx={ { justifyContent: 'center', alignItems: 'center', width: '100%', textAlign: { xs: 'center', md: 'left' } } }
                          item
                          md={ 6 }
                        >
                      ...
                      >
                        <Grid
                          sx={ { paddingX: { md: '15px', xs: '0px' }, textAlign: { xs: 'center', md: 'left' }, width: '100%' } }
                          item
                          md={ 6 }
                        >
                          <Typography sx={ { width: '100%', display: 'flex', justifyContent: { xs: 'center', md: 'left' } } } variant="h5">
                            ID Proof Back:
                          </Typography>
                          <Typography style={ { color: 'black', marginTop: '7px' } }>
