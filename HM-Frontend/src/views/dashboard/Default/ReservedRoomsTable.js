import { Box, Grid, Paper, Slider, Tooltip, Typography } from '@mui/material';
...
const ReservedRoomView = () => {
  ...
  const fetchReservationData = async () => {
    try {
      // const response = await getApi(`api/reservation/viewactiveandcompletedreservation/${hotel._id}`);
  ...
  reservationData?.forEach((item) => {
    ...
    if (checkInDate.month() === currentMonth) {
      reservedRoomMap[checkInDateString] = (reservedRoomMap[checkInDateString] || 0) + 1;
      totalAmt += item.totalPayment;
      console.log(totalAmt, 'totalAmt');
      console.log(item.totalPayment);
    }
  });
    ...
    <Grid container justifyContent="center" spacing={2}>
      <Grid item xs={12}>
        <Typography variant="h5" gutterBottom>
          Reserved Rooms - {moment.months()[currentMonth]} {new Date().getFullYear()} ( {totalReservedRoomsInMonth} rooms reserved ) - ${' '}
          {totalAmt}
        </Typography>
      </Grid>
