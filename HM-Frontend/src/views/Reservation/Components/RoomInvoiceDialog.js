...
const RoomInvoiceDialog = ( props ) => {
  ...
  const initialValues = {
    ...
    roomRent: data?.totalPayment,
    advanceAmount: data?.advanceAmount,
    pendingAmount: data?.totalAmount,
    discount: '',
    gstPercentage: hotelData?.roomgstpercentage,
    haveGST: false,
    gstNumber: '',
    totalAmount: '',
    paymentMethod: 'cash'
  ...
  const generateRoomInvoice = async ( values, resetForm ) => {
    try {
      values.type = 'room';
      ...
      if ( !hotelData.roomgstpercentage ) {
        const data = {};
        data.roomgstpercentage = formik.values.gstPercentage;
      } else {
        console.log( '' );
      }
  ...
  const formik = useFormik( {
    ...
    onSubmit: async ( values, { resetForm } ) => {
      generateRoomInvoice( values, resetForm );
    }
  } );
  ...
  const calculateTotalAmount = () => {
    const totalAmountWithoutGst = formik.values.roomRent - formik.values.discount;

    const gstAmount = totalAmountWithoutGst * ( formik.values.gstPercentage / 100 );
    setGstAmount( gstAmount );
    const totalAmount = haveGST ? totalAmountWithoutGst + gstAmount : totalAmountWithoutGst;
  ...
  useEffect( () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ formik.values.pendingAmount, formik.values.discount, formik.values.gstPercentage, haveGST, formik.values.advanceAmount ] );
            ...
            <Grid container rowSpacing={ 3 } columnSpacing={ { xs: 0, sm: 2 } }>
              ...
              <Grid item xs={ 12 } sm={ 6 }>
                ...
                <TextField
                  ...
                  disabled={ formik.values.roomRent <= 0 || haveGST }
                />
              </Grid>
              <Grid item xs={ 12 } sm={ 6 }>
...
RoomInvoiceDialog.propTypes = {
  open: PropTypes.bool.isRequired,
  handleClose: PropTypes.func.isRequired,
  data: PropTypes.object.isRequired
};

export default RoomInvoiceDialog;
